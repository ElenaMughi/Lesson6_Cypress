...
describe("PositiveTest1", () => {
  ...
  it("VeiwDownloadBookPositive", () => {
    cy.visit("/");
    cy.loginIn("test@test.com", "test");
    cy.addBook("А и Б Стругацкие", "Понедельник начинается в субботу", "фантастический роман", false);
...
describe("NegativeTest", () => {
  it("Positive2", () => {
    ...
    cy.get('#mail').then(($element) => {return $element[0].checkValidity()}).should('be.false');
  });
});